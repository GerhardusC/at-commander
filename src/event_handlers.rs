...
pub fn register_event_handlers(
    ...
    event_loop.on(WifiEvent::Configure, move |_e| {
    ...
    event_loop.on(WifiEvent::Reset, move |_e| {
    ...
    let mut port_cp = port.try_clone()?;
    let read_buffer_cp = read_buffer.clone();
    event_loop.on(WifiEvent::ConnAck, move |_e| {
        if let Ok(mut read_buffer) = read_buffer_cp.lock() {
            read_buffer.clear();
        };
    ...
    let mut port_cp = port.try_clone()?;
    let read_buffer_cp = read_buffer.clone();
    event_loop.on(WifiEvent::Publish, move |e| {
        if let Ok(mut read_buffer) = read_buffer_cp.lock() {
            read_buffer.clear();
        };
    ...
    let read_buffer_cp = read_buffer.clone();
    event_loop.on(WifiEvent::AckReceived, move |_e| {
        if let Ok(mut read_buffer) = read_buffer_cp.lock() {
            read_buffer.clear();
        };
    });
    ...
    let mut port_cp = port.try_clone()?;
    let read_buffer_cp = read_buffer.clone();
    event_loop.on(WifiEvent::Close, move |_e| {
        if let Ok(mut read_buffer) = read_buffer_cp.lock() {
            read_buffer.clear();
        };
        // Tell how many bytes will be sent
